mergeInto(LibraryManager.library, {
    ...
    PostJSON: function(path, id, name, time, objectName, callback, fallback) {
        var parsedPath = Pointer_stringify(path);
        var parsedId = Pointer_stringify(id);
        var parsedName = Pointer_stringify(name);
        var parsedTime = Pointer_stringify(time);
        var parsedObjectName = Pointer_stringify(objectName);
        var parsedCallback = Pointer_stringify(callback);
        var parsedFallback = Pointer_stringify(fallback);
        ...
        try {
            ...
            firebase.database().ref(parsedPath).set({
                id: parsedId,
                name: parsedName,
                time: parsedTime,
            }).then(function(unused) {
                unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, "Success was posted to " + parsedPath);
            });
